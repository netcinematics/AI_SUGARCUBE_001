...
export function initBrainViz() {
    ...
    window.activeStrata = { CORE: false, AURA: false, ATMOS: false }; // Track Strata State
    ...
    let ORBZ_0_SLM_64 = [
        ...
        "aHUMANZa", "aFOGZa", "aPRYZMa", "ALPHABITZA", "BITZ",
        "aMINDzEYEa", "EYEa", "aGEARZa", "aGOGZa", "aLOOPZa",
        "aFLOWZa", "aPATHZa", "aDAWNZa", "aDUSKZa", "aFRICTZa",
        "aBEYONDUs", "aSYNTHaMENTZa", "aSYMBIOZa", "aBLOXYa", "aKNOTZa",
        "aTWIZTa", "aBITZATOMZa", "aPRYZMa", "aDYNAMIXa", "aSTATIXa",
        "aREASONANZa", "aBOOKZa", "aENaSPINZa", "aFALZa", "aDARKa",
        "aNAMEROLOGY", "aSOLARZaSPHEREZa", "aFACTZa", "aLOOPZaFOCOa"
    ];
    ...
    // aSOCIOaNOIZEa	Causes_friction_in	aLIMINa	AXI_ATMOS
    ...
    // aDISaSPINZa	Leads_to	aDISTROPYa	AXI_ATMOS
    ...
    const knowledgeGraph1 = [
        ...
        { src: "aSOCIOaNOIZEa", rel: "Causes_friction_in", dst: "aLIMINa", "strata": "AXI_ATMOS" },
        { src: "aLOOPZaFOCOa", rel: "Traps_the", dst: "Squirrel_Mind", "strata": "AXI_CORE" },
        ...
        { src: "aDISaSPINZa", rel: "Leads_to", dst: "aDISTROPYa", "strata": "AXI_ATMOS" },
        { src: "aPRYZMa", rel: "Refracts_the", dst: "aLUMENTZa", "strata": "AXI_AURA" },
    ...
    // Origin (Source),Relationship (Trax),Destination (Target),Strata (Phase)
    // aBOOKZa,Encapsulates,aZEPTZa_DENSE,AXI_ATMOS
    ...
    // aFRICTZa,Obscures_the,aBOOKZa,AXI_ATMOS
    // aLUMENTZa,Reflects_off,MEMZ,AXI_ASTRO
    // aKNOTZa,Interrupts,aSYMBIOZa,AXI_CORE    

    const knowledgeGraph2 = [
        { src: "aBOOKZa", rel: "Encapsulates", dst: "aZEPTZa_DENSE", strata: "AXI_ATMOS" },
        ...
        { src: "aFRICTZa", rel: "Obscures_the", dst: "aBOOKZa", strata: "AXI_ATMOS" },
    ...
    // const AXI_STRATA = {
    //     CORE: { r: 2, color: 0xFFFFFF, label: "aCORE_AXI" },     // The White Room / Source
    //     AURA: { r: 5, color: 0xFFD700, label: "aAXI_AURA" },    // The Solar Flare Zone
    //     ATMOS: { r: 9, color: 0x00FFFF, label: "aAXI_ATMOS" },  // The Vocabulary Flow
    //     COSMOS: { r: 14, color: 0x4B0082, label: "aAXI_COSMOS" }, // The Black Sphere Floor
    //     ASTRO: { r: 20, color: 0x000011, label: "aAXI_ASTRO" }  // The BeyondUs
    // };
        ...
        for (let x = 0; x < size; x++) {
            for (let y = 0; y < size; y++) {
                for (let z = 0; z < size; z++) {
                    ...
                    // 3. ATMOS_ORBZ (The HUI/Vocabulary Shell) - Radius >= 3.8
                    else {
                        strata = "ATMOS";
                        word = getSeedFromIndex("ATMOS", orbs.length);
                    }
    ...
    function add_ORBZ_512_CUBE() {
        ...
        orbsData.forEach(data => {
            ...
            if (data.strata === "ATMOS") color = 0x7DF9FF; // ATMOS (Electric Blue / Whiter Neon)
    ...
    //     const orbsData = generateAxiEgg512();
    //     const sphereGeometry = new THREE.SphereGeometry(0.18, 16, 16);

    //     orbsData.forEach(data => {
    //         const color = AXI_STRATA[data.strata].color;
    //         const material = new THREE.MeshBasicMaterial({
    //             color: color,
    //             transparent: true,
    //             opacity: data.strata === "CORE" ? 1.0 : 0.5
    //         });
    ...
    //             word: data.word,
    //             strata: data.strata,
    //             buoyancy: data.buoyancy,
    //             vibeFreq: data.vibeFreq,
    //             initialPos: sphere.position.clone()
    //         };

    //         scene.add(sphere);
    //         spheres.set(sphere.id, {
    //             txt: data.word,
    //             ...data.pos,
    //             originalColor: color,
    ...
    //     // Add the "Black Sphere" Portal at the bottom
    //     const portalGeo = new THREE.SphereGeometry(3, 32, 32);
    //     const portalMat = new THREE.MeshBasicMaterial({ color: 0x000000, side: THREE.BackSide });
    ...
    //         if (obj.isMesh && obj.userData.strata) {
    //             const u = obj.userData;

    //             // 1. aREASONANZa Pulse (The Heartbeat)
    //             const pulse = 1 + Math.sin(time * (u.strata === "CORE" ? 4 : 1)) * 0.1;
    //             obj.scale.set(pulse, pulse, pulse);

    //             // 2. Solar Flare Jitter
    //             // Lower Y (High Pressure) has more friction/jitter
    //             const jitterMagnitude = (1 - u.buoyancy) * 0.05;
    //             obj.position.x = u.initialPos.x + (Math.random() - 0.5) * jitterMagnitude;
    //             obj.position.z = u.initialPos.z + (Math.random() - 0.5) * jitterMagnitude;

    //             // 3. Laminar Flow Drift
    ...
    function show_META_NETS(sourceSphere) {
        ...
        // 1. Cleanup previous nets
        activeNetVisuals.forEach(item => {
            scene.remove(item.object);
            // Dispose geometries/materials if needed to avoid leaks
            if (item.object.geometry) item.object.geometry.dispose();
            // if (item.object.material) item.object.material.dispose(); // careful if shared
        });
        ...
        // 2. Find connections
        const connections = all_GRAPHZ.filter(edge => edge.src === srcWord);
        ...
        connections.forEach(edge => {
            // Find destination sphere
            let targetSphereId = null;
            let targetData = null;

            // Search spheres map (values) for matching word
            for (const [id, data] of spheres.entries()) {
                if (data.txt === edge.dst) {
                    targetSphereId = id;
                    targetData = data;
                    break;
                }
            ...
            if (targetData) {
                const targetSphere = scene.getObjectById(targetSphereId);

                // A. Draw Line
                const points = [];
                points.push(sourceSphere.position.clone());
                points.push(targetSphere.position.clone());

                const geometry = new THREE.BufferGeometry().setFromPoints(points);

                // Dynamic Color based on Strata or Relation? Let's use Gold/Yellow for visibility
                const lineMaterial = new THREE.LineBasicMaterial({
                    color: 0xFFD700, // Gold
                    transparent: true,
                    opacity: 0.8,
                    blending: THREE.AdditiveBlending,
                    linewidth: 4 // Note: linewidth doesn't work on all WebGL drivers
                });

                const dir = new THREE.Vector3().subVectors(targetSphere.position, sourceSphere.position);
                ...
                line.position.copy(sourceSphere.position).add(dir.clone().multiplyScalar(0.5));
                line.quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), dir.clone().normalize());
                scene.add(line);
                activeNetVisuals.push({ type: 'line', object: line });

                // B. Show Label on Target
                const labelText = `${edge.rel} -> ${edge.dst}`;
                const labelTex = custom_getLabelTexture(labelText, '#FFD700');
                const spriteMat = new THREE.SpriteMaterial({ map: labelTex, transparent: true });
                const sprite = new THREE.Sprite(spriteMat);

                // Position sprite slight above target
                sprite.position.copy(targetSphere.position);
                sprite.position.y += 0.5; // Offset
                sprite.scale.set(4, 1, 1); // Adjust scale based on text width aspect roughly

                scene.add(sprite);
                activeNetVisuals.push({ type: 'sprite', object: sprite });

            } else {
                console.warn(`Target orb '${edge.dst}' not found in scene for edge from '${srcWord}'.`);
            }
        });
    }
        ...
        idsToDeselect.forEach(id => {
            ...
            if (sphere && data) {
                ...
                if (window.activeStrata && window.activeStrata[data.strata]) {
                    ...
                    else if (data.strata === "ATMOS") { scaleVal = 1.5; opacityVal = 0.5; }
